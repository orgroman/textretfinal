...
def main():
    ...
    if os.path.exists(args.output):
        with open(args.output, 'r') as f:
            for line in f:
                parts = line.split()
                if len(parts) >= 1 and parts[0] != 'qid': # valid line
                    existing_qids.add(parts[0])
    ...
    with open(args.output, 'a') as f_out:
        for line in tqdm(lines, desc="Inference"):
            ...
            # Write results for this query immediately
            for rank, docid in enumerate(final_list, start=1):
                score = 1000.0 - rank
                f_out.write(f"{qid} Q0 {docid} {rank} {score:.4f} rankzephyr\n")
            f_out.flush()
